...
export default function TypewriterText() {
  ...
  return (
    <div className="space-y-4 select-none">
      {/* Main Heading — ghost text reserves full height to prevent layout shift */}
      <h1 className="relative text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-100 leading-tight">
        {/* Invisible ghost text to pre-allocate the final height */}
        <span className="invisible" aria-hidden="true">{fullLine1}</span>
      ...
      {/* Role Subheading — ghost text reserves height, invisible until phase reaches line2 */}
      <p
        className="relative text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-orange-400 via-amber-400 to-orange-500 bg-clip-text text-transparent"
        style={{ visibility: phase === 'line1' ? 'hidden' : 'visible' }}
      >
        {/* Invisible ghost text to pre-allocate the final height */}
        <span className="invisible" aria-hidden="true">{role}</span>
        {/* Visible typewriter text overlaid on top */}
        <span className="absolute inset-0 bg-gradient-to-r from-orange-400 via-amber-400 to-orange-500 bg-clip-text text-transparent">
          {displayed2 || '\u200B'}
          {phase === 'line2' && (
